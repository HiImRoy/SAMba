...
class MFS(nn.Module):
    def __init__(self, embedding_dim):  # 文中embedding_dim=8
        super(MFS, self).__init__()

        # c4 是来自 SAVSS out_indices[0] 的输出 (通道128) -> embedding_dim
        # c3 是来自 SAVSS out_indices[1] 的输出 (通道64) -> embedding_dim
        # c2 是来自 SAVSS out_indices[2] 的输出 (通道32) -> embedding_dim
        # c1 是来自 SAVSS out_indices[3] 的输出 (通道16) -> embedding_dim
        ...
        self.GBC_C = GBC(embedding_dim*4) # 8*4=32 用于concat后的GBC
